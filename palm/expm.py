...
class MatrixExponential(object):
    ...
    def expv(self, t, A, v):
        ...
        if n <= self.krylov_dimension:
            raise ValueError("n should be bigger than krylov dimension")
        ...
        H = numpy.zeros((self.krylov_dimension+2, self.krylov_dimension+2), dt)
        # never overwritten!
        H[self.krylov_dimension + 1, self.krylov_dimension] = 1
        # orthonormal basis for the Krylov subspace + one extra vector
        V = numpy.zeros((n, self.krylov_dimension+1), dt)
        # results
        W = numpy.empty((len(tt), len(v)), dt)
        t = 0  # current time
        beta = v_norm
